...
function lightBalls(world, meshes) {
    ...
    function createAnimation(group) {
        ...
        tl.set(group.position, {
            x: THREE.MathUtils.randInt(-2, 2) * radius + radius * 0.5,
            z: THREE.MathUtils.randInt(-2, 2) * radius + radius * 0.5,
        })
            .to(group.position, 2, {
                y: 18,
...
class Light {
    ...
    createLights() {
        for (let i = 0; i < 3; i++) {
            ...
            const light = new THREE.PointLight(0xfb0692);
            light.intensity = 4.0;
            light.distance = 6;
            light.decay = 1.0;
            group.add(light);
...
const thr = new Hole('', document.querySelector('.testtest'));
